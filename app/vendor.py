'''
NETWORK_TOOL -- Specific Vendor Command Handler
'''
...
def vendor_command_handler(
        ...
        reloading:bool, error_reload:bool, main_thread_lock:Lock,
        precommand:bool=False
):
                ...
                if v in output_dir and v in command:
                    ...
                    with main_thread_lock:
                        print(
                            ...
                            f'Performing command [{fname}{command}] '
                            f'{colorize("|SKIPPED|", "yellow")}, {v} already '
                            'present in device flash'
                        )
                    output_command:str = (
                    ...
                    break
    else:  # precommand == False, post-command
        if (
            command.lower().startswith('copy') or
            command.lower().startswith('do copy')
        ):
            if 'destination filename' in output_command.lower():
                output_command += '\n'
            ...
            if 'save' in output_command.lower():
                ...
                output_command += 'no\n'
            try:   
                net_connect.send_command('', read_timeout=command_timeout)
            except Exception:
            ...
            if 'era' in command.lower():  # wr erase
                ...
                net_connect.send_command_timing('', read_timeout=10)
        elif (
